...
// check afterwards if all doors are inactive
// if all doors are inactive, we need to show the first one on mobile (otherwise no door will be visible on mobile)
$aaForJs['all_doors_are_inactive'] = 1;

// create door classes from given date variables
for ($counter = 1; $counter <= __c('door_amount'); $counter++)
{
    $start_time_conf = __c('door_' . $counter . '_validity_period_start');
    $end_time_conf   = __c('door_' . $counter . '_validity_period_end');

    $start_date = new DateTime($start_time_conf, new DateTimeZone($aa_default_timezone));
    $end_date   = new DateTime($end_time_conf, new DateTimeZone($aa_default_timezone));
    ...
    if ($start_date == $current_date || ($start_date <= $current_date && $end_date >= $current_date))
    {
        ...
        $aaForJs['all_doors_are_inactive'] = 0;
    }
    $aaForJs['config']['door_' . $counter . '_class']['value'] = $door_class;
    //pr($counter . ' - ' . $start_date->format("d-m-Y") . ' - ' . $end_date->format("d-m-Y") . ' - ' . $door_class);
}
...
$aaForJs['custom']['admin_key'] = md5($aa_inst_id . '_' . $aa['instance']['aa_inst_secret']);